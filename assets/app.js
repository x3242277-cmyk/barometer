...
/* איחודי הרשימות של 2026: מי שרץ יחד היום, נספר יחד במודל */
const MERGERS_2026 = [
  { into: "labor",             name: "הדמוקרטים (העבודה + מרצ)",              from: ["labor", "meretz"] },
  { into: "hadash_taal",       name: "הרשימה המשותפת (חד״ש–תע״ל + בל״ד)",     from: ["hadash_taal", "balad"] },
  { into: "religious_zionism", name: "הציונות הדתית–עוצמה + הבית היהודי",     from: ["religious_zionism", "jewish_home"] }
];

function demoPartyList(merged) {
  const D = S.demo;
  if (!merged) return D.parties2022.map(p => ({ ...p }));
  const byId = Object.fromEntries(D.parties2022.map(p => [p.id, p]));
  const absorbed = new Set();
  const out = [];
  D.parties2022.forEach(p => {
    if (absorbed.has(p.id)) return;
    const m = MERGERS_2026.find(x => x.into === p.id);
    if (!m) { out.push({ ...p }); return; }
    const parts = m.from.map(id => byId[id]).filter(Boolean);
    const votes = parts.reduce((s, x) => s + x.votes, 0);
    const mix = {};
    parts.forEach(x => Object.entries(x.mix).forEach(([k, v]) => { mix[k] = (mix[k] || 0) + v * x.votes / votes; }));
    parts.forEach(x => absorbed.add(x.id));
    out.push({ id: p.id, name: m.name, votes, seats: parts.reduce((s, x) => s + x.seats, 0), camp: p.camp, mix, merged: true, parts: parts.map(x => x.name) });
  });
  return out;
}

function runDemoModel(years, merged = S.demoMerged !== false) {
  const D = S.demo, P = demoParams();
  const base = Object.fromEntries(D.sectors.map(s => [s.id, s.turnout]));
  const list = demoPartyList(merged);
  const votes = {};
  list.forEach(p => {
    let f = 0;
  ...
  const pairs = S.demo.surplusAgreements.filter(([a, b]) => passing[a] != null && passing[b] != null);
  const seats = baderOfer(passing, pairs, 120);
  return { list, byId: Object.fromEntries(list.map(p => [p.id, p])), votes, totalValid, thr, passing, seats,
           failed: Object.keys(votes).filter(k => !(k in passing)), merged };
}

function renderDemography() {
  const D = S.demo;
  const now = runDemoModel(D.meta.years), base = runDemoModel(0), base22 = runDemoModel(0, false);
  const byId = now.byId;
  const campSeats = {}; Object.keys(D.camps).forEach(c => campSeats[c] = 0);
  Object.entries(now.seats).forEach(([id, n]) => campSeats[byId[id].camp] += n);
  const campSeats22 = {}; Object.keys(D.camps).forEach(c => campSeats22[c] = 0);
  Object.entries(base.seats).forEach(([id, n]) => campSeats22[byId[id].camp] += n);

  const order = ["right", "haredi", "arab", "center"];
  const items = [];
  order.forEach(c => Object.entries(now.seats).filter(([id]) => byId[id].camp === c)
    .sort((a, b) => b[1] - a[1])
    .forEach(([id, n]) => items.push({ color: D.camps[c].color, count: n, label: `${byId[id].name} · ${n}` })));
  const bloc = campSeats.right + campSeats.haredi;
  $("#demo-hemi").innerHTML = `<div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:10px">
    ...
    hemicycleSVG(items, { aria: "תחזית דמוגרפית" });
  $("#demo-blocbar").innerHTML = blocBarHTML(order.map(c => ({ count: campSeats[c], color: D.camps[c].color, label: `${D.camps[c].name}: ${campSeats[c]}` })));
  $("#demo-legend").innerHTML = order.map(c =>
    `<span style="display:inline-flex;align-items:center;gap:8px;font-size:.83rem;font-weight:600"><i style="width:11px;height:11px;border-radius:3px;background:${D.camps[c].color}"></i><b class="num">${campSeats[c]}</b> ${esc(D.camps[c].name)} <span dir="ltr" style="color:var(--ink-3)">(${campSeats[c] - campSeats22[c] >= 0 ? "+" : ""}${campSeats[c] - campSeats22[c]})</span></span>`).join("");

  const totalGrowth = 100 * (Object.values(now.votes).reduce((a, b) => a + b, 0) / Object.values(base.votes).reduce((a, b) => a + b, 0) - 1);
  const campVotes = {}; order.forEach(c => campVotes[c] = 0);
  Object.entries(now.votes).forEach(([id, v]) => { campVotes[byId[id].camp] += v; });
  const campShare = c => 100 * campVotes[c] / now.totalValid;      // כולל קולות מתחת לאחוז החסימה
  const campProp  = c => campShare(c) / 100 * 120;                  // זכאות פרופורציונלית טהורה
  const wastedSeats = order.reduce((s, c) => s + Math.abs(campProp(c) - campSeats[c]), 0) / 2;
  $("#demo-delta").innerHTML = `<div style="margin-top:8px">
      ...
      <span style="color:var(--ink-3);font-size:.8rem">מנדטים לגוש הימני–חרדי · בפועל ב־2022: 64</span></div>
    <table style="width:100%;min-width:0;margin-top:16px;border-collapse:collapse;font-size:.8rem;font-variant-numeric:tabular-nums">
      <thead><tr style="color:var(--ink-3);font-size:.72rem;font-weight:700">
        <th style="text-align:start;padding:4px 2px">גוש</th>
        <th style="text-align:end;padding:4px 2px">% קולות</th>
        <th style="text-align:end;padding:4px 2px">זכאות יחסית</th>
        <th style="text-align:end;padding:4px 2px">בפועל</th></tr></thead>
      <tbody>${order.map(c => {
        const sh = campShare(c), prop = campProp(c), act = campSeats[c], gap = act - prop;
        const gtag = Math.abs(gap) < 0.05 ? `<span style="color:var(--ink-3);font-size:.9em">±0</span>`
          : `<span dir="ltr" style="color:${gap > 0 ? "#1F6349" : "#8E241A"};font-size:.9em">${gap > 0 ? "+" : "−"}${r1(Math.abs(gap))}</span>`;
        return `<tr style="border-top:1px solid var(--rule-2)">
          <td style="padding:7px 2px"><i style="display:inline-block;width:9px;height:9px;border-radius:2px;background:${D.camps[c].color};margin-inline-end:6px;vertical-align:middle"></i>${esc(D.camps[c].name)}</td>
          <td style="text-align:end;padding:7px 2px">${sh.toFixed(1)}%</td>
          <td style="text-align:end;padding:7px 2px;color:var(--ink-3)">${prop.toFixed(1)}</td>
          <td style="text-align:end;padding:7px 2px"><b>${act}</b> ${gtag}</td></tr>`;
      }).join("")}</tbody>
    </table>
    <p style="margin:14px 0 0;color:var(--ink-2);font-size:.82rem"><b>זכאות יחסית</b> = חלֵק הקולות של הגוש × 120, כולל קולות שירדו לטמיון. <b>בפועל</b> = תוצאת בדר־עופר: אחוז החסימה והסכמי העודפים מזיזים בסך הכול ${r1(wastedSeats) === 1 ? "<b>מנדט אחד</b>" : `כ־<b>${r1(wastedSeats)}</b> מנדטים`} בין הגושים לעומת חלוקה פרופורציונלית טהורה.</p>
    <p style="margin:10px 0 0;color:var(--ink-2);font-size:.82rem">אותו מבנה רשימות, אותם הרגלי הצבעה — רק אוכלוסייה בגודל של ${D.meta.targetYear}. סך הקולות הכשרים גדל ב־<b>${r1(totalGrowth)}%</b> ב־${D.meta.years} שנים, אך לא באופן אחיד — וזה כל הסיפור.</p>
    ${now.failed.length ? `<p style="margin:10px 0 0;color:var(--ink-3);font-size:.78rem">מתחת לאחוז החסימה במודל: ${now.failed.map(id => esc(byId[id].name)).join(", ")}.</p>`
      : `<p style="margin:10px 0 0;color:#1F6349;font-size:.78rem">בתרחיש הזה אף רשימה אינה נופלת מתחת לאחוז החסימה — אף קול לא הולך לאיבוד.</p>`}`;

  const thrPct = 100 * D.meta.threshold;
  const share22 = id => base.votes[id] != null ? 100 * base.votes[id] / base.totalValid : null;
  $("#demo-parties").innerHTML = Object.entries(now.votes).sort((a, b) => b[1] - a[1]).map(([id, v]) => {
    const p = byId[id], col = D.camps[p.camp].color;
    const share = 100 * v / now.totalValid, n = now.seats[id] || 0;
    const passed = id in now.passing, s22 = share22(id), dPP = s22 == null ? null : share - s22;
    const dSeat = n - (base.seats[id] || 0);
    const ppTxt = dPP == null ? "" : `<span style="color:${dPP > 0.05 ? "#1F6349" : dPP < -0.05 ? "#8E241A" : "var(--ink-3)"}">${dPP > 0.05 ? "▲" : dPP < -0.05 ? "▼" : "±"}‏ ${r1(Math.abs(dPP))} נק׳</span>`;
    const sub = passed
      ? `<b style="color:var(--ink-2)">${n} מנדטים${dSeat ? ` (${dSeat > 0 ? "+" : ""}${dSeat})` : ""}</b> · ${ppTxt} מ־2022 · ב־2022 ${s22 == null ? "—" : pct(s22)}${p.merged ? " (יחד)" : ""}`
      : `<b style="color:var(--ink-2)">0 מנדטים</b> · מתחת לאחוז החסימה (${r1(thrPct)}%), חסר ${r1(thrPct - share)} נק׳ · ${ppTxt} מ־2022`;
    return `<div class="prow" style="--c:${col}${passed ? "" : ";opacity:.62"}">
      <span class="orglogo" style="background:${col};color:#fff;border:0">${esc(initials(p.name))}</span>
      <div style="min-width:0"><div class="pname">${esc(p.name)}</div><div class="psub">${sub}</div>${passed ? "" : `<span class="tagfix">קולות שיורדים לטמיון</span>`}</div>
      <div class="pbar" style="--c:${col};--w:${clamp(share / 26 * 100)}%"><i></i></div>
      <div class="pseats num" style="${passed ? "" : "color:var(--ink-3)"}">${share.toFixed(1)}<span style="font-size:.46em;font-weight:700">%</span></div></div>`;
  }).join("");

  /* הערת מבנה הרשימות */
  const mergedList = MERGERS_2026.map(m => m.name).join(" · ");
  $("#merge-note").className = "notice " + (now.merged ? "" : "warm");
  $("#merge-note").querySelector("p").innerHTML = now.merged
    ? `<b>המודל רץ לפי איחודי 2026.</b> מרצ והעבודה רצות יחד כ"הדמוקרטים", חד״ש–תע״ל ובל״ד כ"רשימה המשותפת", והבית היהודי בתוך הציונות הדתית. לכן ${fmt(S.demo.parties2022.find(p => p.id === "meretz").votes + S.demo.parties2022.find(p => p.id === "balad").votes)} הקולות שירדו לטמיון ב־2022 <b>נספרים כאן במלואם</b> — וזו הסיבה שהתמונה שונה מ־2022 גם לפני שנגענו בדמוגרפיה.`
    : `<b>מבחן שפיות:</b> במבנה של 2022 ובגידול אפס, המודל משחזר את התוצאה מנדט במנדט — 32 לליכוד, 24 ליש עתיד, 4 לעבודה — כולל בדר־עופר והסכמי העודפים שנחתמו בפועל. מרצ, בל״ד והבית היהודי נופלות מתחת לאחוז החסימה, בדיוק כפי שקרה.`;
  $$("[data-merge]").forEach(b => b.setAttribute("aria-pressed", String((b.dataset.merge === "1") === (S.demoMerged !== false))));
...
function wire() {
  ...
  $$("[data-merge]").forEach(b => b.addEventListener("click", () => { S.demoMerged = b.dataset.merge === "1"; renderDemography(); }));
...
function renderHaredi() {
  ...
  $("#haredi-verdict").innerHTML = `
    ...
    <p style="margin:12px 0 0;color:var(--ink-3);font-size:.78rem">שתי דרכים שונות לחשב, ושתיהן מגיעות לאותו אזור: החישוב מהמגזר שלמעלה נותן ${r1(st.total)} מנדטים, ו<a href="#/demography" style="color:var(--navy)">המודל הדמוגרפי המלא</a> — שמריץ בדר־עופר על כל הרשימות — נותן ${modelSum}. אף אחד מהם לא נותן את ${r1(pollSum)} שהסקרים אומרים.</p>`;
