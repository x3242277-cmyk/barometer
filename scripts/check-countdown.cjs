...
const boxes=[0,1].map(i=>{const classes=new Set(['night-waiting']);return {hidden:false,innerHTML:'',classes,closest(){return {id:i?'view-results':'view-live',classList:{toggle(name,on){if(on)classes.add(name);else classes.delete(name);}}}}};});
...
assert(boxes[0].innerHTML.includes('live-timer-row')&&!boxes[0].innerHTML.includes('בינתיים, לתמונת המצב'));
assert(boxes[1].innerHTML.includes('בינתיים, לתמונת המצב'));
