...
function renderHomeHemicycle(seats, blocTot, est) {
  ...
  /* שיעור הקולות המשוער: אומדני התחזית (אחרי ההנחות) + הרשימות שמתחת לסף מהממוצע
     הגולמי. ההבדל מחלוקת המנדטים הוא בדיוק הקולות שאובדים מתחת לאחוז החסימה. */
  const parties = est?.parties || {}, rawFull = est?.rawFull || {};
  const belowIds = Object.keys(rawFull).filter(id => parties[id] == null && rawFull[id] > 0);
  const votes = { ...Object.fromEntries(belowIds.map(id => [id, rawFull[id]])), ...parties };
  const sumV = Object.values(votes).reduce((t, v) => t + v, 0) || 1;
  const shareOf = pred => 100 * Object.entries(votes).filter(([id]) => pred(sideOf(id))).reduce((t, [, v]) => t + v, 0) / sumV;
  const shR = shareOf(sd => sd === "right"), shU = shareOf(sd => sd === "mid"), shL = shareOf(sd => sd === "left");
  /* מי בפועל בתוך ה-shU: רשימות אמיתיות (עם מנדטים) ש-sideOf שם ל"אמצע" —
     היום זו הרשימה המשותפת, לא רע״מ (רע״מ ב"אמצע" הוויזואלי של הקשת דרך
     ARAB_FAMILY, אבל מנדטיה נספרים ב-shL כי sideOf מכבד את השיוך הידני שלה
     ל-Left). אם אין רשימה אמיתית כזו, כל ה-shU הוא קולות מתחת לאחוז החסימה. */
  const midRealIds = ids.filter(id => sideOf(id) === "mid");
  const midWhat = midRealIds.length ? midRealIds.map(id => partyMeta(id).name).join(", ") : "רשימות מתחת לאחוז החסימה";
  ...
  box.innerHTML = `<div class="seatmap">
    ...
    <div class="sm-votes"><div class="sm-votes-head"><span>שיעור הקולות המשוער לכל גוש · כולל רשימות שמתחת לאחוז החסימה, שקולותיהן אובדים${wasted.length ? ` (${esc(wasted.join(", "))})` : ""}</span></div>
      <div class="sm-votes-row"><b class="num">${r1(shR)}%</b><div class="sm-bar thin"><span style="width:${shR}%;background:${BLOCS.Right.color}"></span>${shU > 0.05 ? `<span style="width:${shU}%;background:${BLOCS.Unknown.color}" title="${esc(midWhat)} ${r1(shU)}%"></span>` : ""}<span style="width:${shL}%;background:${BLOCS.Left.color}"></span></div><b class="num">${r1(shL)}%</b></div>
      ${shU > 0.05 ? `<div class="sm-votes-mid"><i style="background:${BLOCS.Unknown.color}"></i>${r1(shU)}% ${esc(midWhat)}</div>` : ""}</div>
  </div>`;
