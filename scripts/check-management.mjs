...
assert.equal((await req('/api/admin/save',{...draft,kind:'sample'})).status,200);assert.equal((await req('/api/content/live-results.json')).data.sample.parties.length,2);
...
console.log('Passed: private login, expiry, cross-origin rejection, poll validation, duplicate prevention, persistent public feed, failed-source preservation, sample save, private analytics and cumulative-event deduplication.');
